...
public class PinyinIME extends InputMethodService {
	...
	private FounctionContainer  mMoreFunctionContainer;
	 ...
	 * 
	 * @see com.hanvon.sulupen.pinyin.PinyinIME.ImeState
	 */
	...
	private boolean connectSuccessState=false;
    ...
    public Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case BluetoothMsgReceive.BT_CONNECTED:
            	mMoreFunctionContainer.getEpenBn().setImageResource(R.drawable.edit_light);
            	break;
            case BluetoothMsgReceive.BT_DISCONNECT:
            	mMoreFunctionContainer.getEpenBn().setBackgroundResource(R.drawable.skb_editor_selector);
            	break;
            case BluetoothChatService.BLUETOOTH_MESSAGE_READ:
				...
				try {
					...
					{
						LogUtil.i("tong----------imageStrs:"+imageStr);
						byte[] imagebyte=Base64.decode(imageStr, Base64.DEFAULT);
	                    LogUtil.i("tong------------unGzip:"+imagebyte.length);
						//ungzip bmpByte
	...
	public void onCreate() {
       ...
       //英文输入状态
		...
		mEnvironment.onConfigurationChanged(getResources().getConfiguration(),
				this);
		ConstPool.SwitchInputMethod=true;
		sendBroadcast(new Intent().setAction(ConstPool.SWITCH_ON_INPUTMETHOD_ACTION));
	}
	...
	public void onDestroy() {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "onDestroy.");
			ConstPool.SwitchInputMethod=false;
			sendBroadcast(new Intent().setAction(ConstPool.SWITCH_OFF_INPUTMETHOD_ACTION));
		}
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (mEnvironment.needDebug()) {
			...
			LogUtil.d( "--last config: " + env.getConfiguration().toString());
			LogUtil.d( "---new config: " + newConfig.toString());
		}
   ...
   public void changeEditText( String newString){
	   System.out.println("tong----------in changeEditText:"+newString);
	   InputConnection ic = getCurrentInputConnection();
	   if (ic == null)
			return;
		//ic.deleteSurroundingText(editString.toString().length(),0);
		//ic.commitText(newString,1);
		//editString.replace(0, editString.toString().length(), newString);
	    sendKeyChar('\n');
		ic.commitText(newString, 1);
		editString.append(newString+'\n');
	    LogUtil.i("change 后："+editString.toString());
   }
	/**
	 * 按键处理函数
	 * 
	...
	private boolean processKey(KeyEvent event, boolean realAction) {
		...
		if (keyCode >= KeyEvent.KEYCODE_A && keyCode <= KeyEvent.KEYCODE_Z) {
			...
			LogUtil.w( "kegydfsfsdfsfsdfsdfsfsdfs");
	       Toast.makeText(this, "hehe", Toast.LENGTH_SHORT).show();
		}
		if (mInputModeSwitcher.isEnglishWithSkb()) {// 英语软键盘处理
			return mImEn.processKey(this,getCurrentInputConnection(), event,
	...
	private boolean processFunctionKeys(int keyCode, boolean realAction) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			 ...
			 if (isInputViewShown()) {
					LogUtil.i("");
					if(mMoreFunctionContainer.popIsShowing()){
		...
		if (null != mCandidatesContainer && mCandidatesContainer.isShown()
			...
			if (keyCode == KeyEvent.KEYCODE_DEL) {
				...
				if (SIMULATE_KEY_DELETE) {
					...
					LogUtil.i("SIMULATE_KEY_DELETE");
					simulateKeyEventDownUp(keyCode);
					LogUtil.i("delete 前:"+editString.toString());
					if(editString.length()>0)
					editString.delete(editString.length()-1,editString.length());
					LogUtil.i(editString.toString());
				} else {
					// 发送删除一个字符的操作给EditText
					getCurrentInputConnection().deleteSurroundingText(1, 0);
					LogUtil.i("delete2 前:"+editString.toString());
					editString.delete(editString.length()-2,editString.length()-1);
					LogUtil.i(editString.toString());
				}
				return true;
			}
	...
	private boolean processStateIdle(int keyChar, int keyCode, KeyEvent event,
		...
		if (keyChar >= 'a' && keyChar <= 'z' && !event.isAltPressed()) {
			...
			if (SIMULATE_KEY_DELETE) {
				...
				LogUtil.i("");
				simulateKeyEventDownUp(keyCode);
				LogUtil.i("ch delete 前："+editString.toString());
				if(editString.length()>0)
				editString.delete(editString.length()-1, editString.length());
				LogUtil.i("ch delete 后："+editString.toString());
			} else {
				// 发送删除一个字符的操作给 EditText
				getCurrentInputConnection().deleteSurroundingText(1, 0);
				LogUtil.i("ch2 delete 前:"+editString.toString());
				editString.delete(editString.length()-2,editString.length()-1);
				LogUtil.i(editString.toString());
...
//	onCreateCandidatesView
	public View onCreateCandidatesView() {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "onCreateCandidatesView.");
		}
		...
		mSettingContainer=(FrameLayout)inflater.inflate(R.layout.setting_container,null);
		mCandidatesContainer = (CandidatesContainer)mSettingContainer.findViewById(R.id.candidates_container); 
		mMoreFunctionContainer = (FounctionContainer)mSettingContainer.findViewById(R.id.layout_moreFunction); 
...
//				R.layout.candidates_container, null);
        ...
        setCandidatesView(mSettingContainer);
		setCandidatesViewShown(true);
		Log.i("onCreateCandidatesView", "onCreateCandidatesView");
		editString=new StringBuffer();
		return mSettingContainer;
	}
	/**
	 *初始化蓝牙笔连接状态灯 
	 */
	 ...
	 *长按删除键的处理
	 */
	public void responseSoftKeyEventDel(SoftKey sKey) {
		LogUtil.i("tong-----------responseSoftKeyEventDel");
		if (null == sKey)
			return;
		InputConnection ic = getCurrentInputConnection();
		if (ic == null)
		...
		LogUtil.i("delete dl:"+editString.toString()+editString.length());

		//如果是中文的候选词列表显示
		if(mInputModeSwitcher.isChineseText())
		{
			LogUtil.i("tong------chinese:"+mComposingView.getVisibility());
	     
			if( mDecInfo.length() == 0)
		   // if (mDecInfo.isCandidatesListEmpty())
		    {
		    	LogUtil.i("tong------is  null");
		        int len = editString.length();
				for(int i=0;i<len;i++)
				{
					LogUtil.i("tong-----------editString"+editString+":"+editString.length());
					ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN, keyCode));
					ic.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_UP, keyCode));
					editString.delete(editString.length()-1,editString.length());
					LogUtil.i("i---"+i);
			...
			else
			{
				...
				//拼音列表不可见,点击提交以后
				if(mComposingView.getVisibility() != 0)
				{
					//隐藏那个候选列表
				...
				int lenCandidate = mDecInfo.length();
	...
	public void responseSoftKeyEvent(SoftKey sKey) {
		...
		if (sKey.isUserDefKey()) {// 是用户定义的keycode
			...
			Log.i("InutModeSwitcher", "keyCode=="+keyCode);
			updateIcon(mInputModeSwitcher.switchModeForUserKey(keyCode,sKey));
			resetToIdleState(false);
			mSkbContainer.updateInputMode();
			mSkbContainer.updateIconChEn(keyCode,sKey);
		} else {
			if (sKey.isKeyCodeKey()) {// 是系统的keycode
			...
			if (!mSkbContainer.isCurrentSkbSticky()) {
				LogUtil.i("tong-------!mSkbContainer.isCurrentSkbSticky()");
	...
	/**
	 * 根据输入法状态更改 候选词框的状态：正在输入状态和设置状态
	 * @param state
	 */
    private void changeCandidateWindowState(ImeState state){
    	if (mEnvironment.needDebug()) {
			LogUtil.d( "changeCandidateWindowState window is done");
		}

    	if(ImeState.STATE_IDLE==state){
    		LogUtil.d( "当前状态："+state.name());
    		mCandidatesContainer.setVisibility(View.INVISIBLE);
    		mMoreFunctionContainer.setVisibility(View.VISIBLE);
    		initBTState();
    	}else{
    		mMoreFunctionContainer.setVisibility(View.INVISIBLE);
	...
	private void showCandidateWindow(boolean showComposingView) {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "Candidates window is shown");
		}
	...
	private void dismissCandidateWindow() {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "Candidates window is to be dismissed");
		}
		if (null == mCandidatesContainer)
			return;
		try {
			...
			LogUtil.e( "Fail to show the PopupWindow.");
		}
	...
	private void resetCandidateWindow() {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "Candidates window is to be reset");
		}
		if (null == mCandidatesContainer)
			return;
		try {
	...
	public void onStartInput(EditorInfo editorInfo, boolean restarting) {
		...
		if(editString==null)
			...
			if(editString.length()!=0)
			editString.delete(0,editString.length());
	...
	public void onStartInputView(EditorInfo editorInfo, boolean restarting) {
		...
		setCandidatesViewShown(true);
	    //扫描记录模式更改
		recordMode=BluetoothService.scanRecordMode;
	...
	public void onFinishCandidatesView(boolean finishingInput) {
		if (mEnvironment.needDebug()) {
			LogUtil.d( "onFinishCandidateView.");
		}
		resetToIdleState(false);
