...
import com.hanvon.sulupen.pinyin.SoftKeyboard.KeyRow;
import com.hanvon.sulupen.pinyin.recongnition.BaiduVoiceRecognition;
import com.hanvon.sulupen.thread.TranslateTask;
...
public class InputModeSwitcher {
	...
	/**
	 * User defined key code, used by soft keyboard. 翻译按键功能。
	 */
	private static final int USERDEF_KEYCODE_TRANS_10 = -10;

	/**
	 * User defined key code, used by soft keyboard. 翻译按键功能。
	 */
	private static final int USERDEF_KEYCODE_TRANS_11 = -11;
	
	/**
	 * User defined key code, used by soft keyboard. 中英按键功能。
	 */
	private static final int USERDEF_KEYCODE_MORE_SYM_8 = -8;
	...
	private boolean mStateFromEn;
	private boolean mStateFromHalf;
	...
	class ToggleStates {
		 ...
		 * {@link com.hanvon.sulupen.pinyin.SoftKeyboard.KeyRow} for
		 * details.
		 */
	...
	public int getSkbLayout() {
		LogUtil.i("tong-------mInputMode is:" +mInputMode);
		int layout = (mInputMode & MASK_SKB_LAYOUT);
		LogUtil.i("tong-------layout is:" +layout);
		switch (layout) {
	...
	public int switchLanguageWithHkb() {
		...
		mInputIcon = R.drawable.chinese_switch;

		if (MODE_HKB_CHINESE == mInputMode) {
			newInputMode = MODE_HKB_ENGLISH;
			mInputIcon = R.drawable.english_switch;
	...
	/**
	 * 特殊图标的处理,注意这时已经跳转到新的界面了，saveInputMode了，需要判断mPreviousInputMode
	 */
	public void switchIconChEn(int userKey,SoftKey sKey) {
		Resources r = mImeService.getResources();  
		//中英文按键
		if(USERDEF_KEYCODE_MORE_SYM_8 == userKey)
        {
			LogUtil.i("tong-------switchIconChEn:"+mPreviousInputMode);
			if (MODE_SKB_CHINESE == mPreviousInputMode) {
				//中文切换英文
				Drawable btnDrawable = r.getDrawable(R.drawable.english_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			} else if (MODE_SKB_ENGLISH_LOWER == mPreviousInputMode
					|| MODE_SKB_ENGLISH_UPPER == mPreviousInputMode) {	
				//英文切换中文
				Drawable btnDrawable = r.getDrawable(R.drawable.chinese_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			    
			} else if (MODE_SKB_SYMBOL1_CN == mPreviousInputMode) {
				//切换半角
				Drawable btnDrawable = r.getDrawable(R.drawable.banjiao_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			
			} else if (MODE_SKB_SYMBOL1_EN == mPreviousInputMode) {
				//切换全角
				Drawable btnDrawable = r.getDrawable(R.drawable.quanjiao_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			} else if (MODE_SKB_SYMBOL2_CN == mPreviousInputMode) {
				//切换半角
				Drawable btnDrawable = r.getDrawable(R.drawable.banjiao_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			} else if (MODE_SKB_SYMBOL2_EN == mPreviousInputMode) {
				//切换全角
				Drawable btnDrawable = r.getDrawable(R.drawable.quanjiao_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			} else if (MODE_SKB_SMILEY == mPreviousInputMode) {
				Drawable btnDrawable = r.getDrawable(R.drawable.chinese_switch); 
			    sKey.setKeyBackSource(btnDrawable);
			}
        }
	 ...
	 * @return the icon to update.
	 */
	public int switchModeForUserKey(int userKey,SoftKey sKey) {
		int newInputMode = MODE_UNSET;
		
        //中英文按键
		if(USERDEF_KEYCODE_MORE_SYM_8 == userKey)
        {
			LogUtil.i("tong-------zhong/ying");
		...
		if (USERDEF_KEYCODE_LANG_2 == userKey) {
			...
			LogUtil.i("tong-------mInputMode:"+mInputMode);
			mStateFromEn = false;
			// 系统键
			if (MODE_SKB_CHINESE == mInputMode) {
				if(mStateFromHalf)
		...
		} else if(USERDEF_KEYCODE_TRANS_10==userKey){
			new TranslateTask(mImeService).execute();
		}else if(USERDEF_KEYCODE_TRANS_11==userKey){
//			new FounctionContainer(mImeService, null).showPopup(ibnVoiceRecon, initVoiceReconView());;
			//显示语音输入的界面
//			showPopu();
		}
	...
	private View initVoiceReconView() {
		...
		View view = LayoutInflater.from(mImeService).inflate(
				R.layout.dlg_voice_input, null);
		final ImageView voiceBar = (ImageView) view
				.findViewById(R.id.volumeProgressbar);
		 voiceRecognition = new BaiduVoiceRecognition(mImeService, mImeService);		
		voiceBar.setOnTouchListener(new OnTouchListener() {
		 ...
		 String languageType = PreferHelper.getString(
	...
	public void showPopu(){
		...
		int popWidth = env.getScreenWidth();
		int popHeight = env.getSkbHeight();
		...
		View mVoiceView=LayoutInflater.from(mImeService).inflate(R.layout.dlg_voice_input, null);
		mPopupWindow.setContentView(mVoiceView);
		// popupWinMenu.showAsDropDown(bn,0, 0, Gravity.CENTER_HORIZONTAL);
		mPopupWindow.showAtLocation(null, Gravity.CENTER_HORIZONTAL
				| Gravity.BOTTOM, 0, 0);
	}
	/**
	...
	public int requestBackToPreviousSkb() {
		LogUtil.i("tong-------requestBackToPreviousSkb mInputMode is:" +mInputMode);
		int layout = (mInputMode & MASK_SKB_LAYOUT);
	...
	public boolean isFromEn() {
		LogUtil.i("isFromEn-----------"+mStateFromEn);
		return mStateFromEn;
	}
	public void setStateFrom(boolean state)
	{
		mStateFromEn = false;
		mStateFromHalf = false;
	}
	...
	private void saveInputMode(int newInputMode) {
		...
		LogUtil.i("tong-------mPreviousInputMode mInputMode is:" +mInputMode);
		// 设置新的输入法模式为当前的输入法模式
		mInputMode = newInputMode;
		LogUtil.i("tong-------newInputMode mInputMode is:" +mInputMode);
		// 输入法模式的布局属性（第8位）
		int skbLayout = (mInputMode & MASK_SKB_LAYOUT);
		if (MASK_SKB_LAYOUT_QWERTY == skbLayout || 0 == skbLayout) {
			// 保存最近的语言输入法模式
		...
		mInputIcon = R.drawable.ime_pinyin;
		if (isEnglishWithHkb()) {
			LogUtil.i("tong-----------isEnglishWithHkb");
			mInputIcon = R.drawable.ime_en;
		} else if (isChineseTextWithHkb()) {
			LogUtil.i("tong-----------isChineseTextWithHkb");
			mInputIcon = R.drawable.ime_pinyin;
		}
