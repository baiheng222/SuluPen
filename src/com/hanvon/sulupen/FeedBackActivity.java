...
import com.hanvon.inputmethod.ui.ConnectionDetector;
...
public class FeedBackActivity extends Activity implements OnClickListener
{
	...
	private String UploadBody(String mPackageName, String version,
	...
	public int feedback(final String content, final String mail)
	{
		try
		{
			...
			mHttpUtils.send(HttpRequest.HttpMethod.POST,
					"http://cloud.hwyun.com/ws-dcloud/rt/ap/v1/statistics/gather/feedback",
							params, new RequestCallBack<String>()
							{
										...
										String arg1)
								{
									...
									mDefaultPreference.edit()
											.putString("feedback_msg", content)
											.commit();
									mDefaultPreference.edit()
											.putString("feedback_mail", mail)
											.commit();
								}
										...
										ResponseInfo<String> responseInfo)
								{
									try
									{
										...
										final String code = mJsonObject
												.getString("code");
										if (code.equals("0"))
										{
											String result = mJsonObject
													.getString("result");
										...
										} else
										{
											...
											mDefaultPreference
													.edit()
													.putString("feedback_msg",
															content).commit();
											mDefaultPreference
													.edit()
													.putString("feedback_mail",
															mail).commit();
										}
	...
	private void commit()
	{
		...
		{
			Toast.makeText(this, R.string.feedback_content_null,Toast.LENGTH_LONG).show();
		} 
		else if (!isMailNO(mEtEmail.getText().toString())
				&& mEtEmail.getText().toString().length() != 0)
		{
			Toast.makeText(this, R.string.feedback_mail_null, Toast.LENGTH_LONG).show();
		} 
		else
		{
			showProgress();
			...
			{
				handler.sendEmptyMessage(0);
				mDefaultPreference.edit().putString("feedback_msg", content.getText().toString()).commit();
				mDefaultPreference.edit().putString("feedback_mail", mail.getText().toString()).commit();
			}
			else
			{
				new Thread()
